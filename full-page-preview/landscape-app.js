...
function render(){
 ...
 document.querySelector('#layer-caption').textContent=layer==='geography'?'地理依据：实际高程、河网、湖泊与历史城镇范围，按原区划示意映射。':layer==='none'?'原 98 区划边界与面积均衡结果。':(mode==='balanced'?'南昌、九江与抚州已采用统一比例、独立生成的手绘底图；其余江西城市正在逐区更新。':'原始布局参考；南昌、九江与抚州独立底图请切换至面积均衡布局查看。');
