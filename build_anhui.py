"""Extend the accepted eastern map with Anhui on the same shared network."""
...
def base():
    ...
    result,report=simplify(initial,real,18,minimum_iou=.82)
    save('data/anhui-base.json',{'paths':[svg_path(p) for p in result],'report':report})
...
def anchor():
    ...
    regions,raw,_=ah_source();real=bodies(raw);current=[geometry_from_path(p) for p in load('data/anhui-base.json')['paths']]
    ...
    ah_outline=unary_union(current).boundary
    junctions=[p for p,n in nodes.items() if n>=3 and ah_outline.distance(Point(p))<1e-6]
    interfaces=[];mapping={}
    ...
    print('Anchored',[(r['name'],round(p.area),round(p.intersection(q).area/p.union(q).area,3),turns(p)) for r,p,q in zip(regions,combined[45:],real)],flush=True)
    assert all(p.equals(q) for p,q in zip(fixed,combined))
    assert shapely.coverage_is_valid(combined)
    assert topology_signature(combined[45:])==topology_signature(real)
    assert all(p.intersection(q).area/p.union(q).area>=.75 and abs(p.area-q.area)/q.area<=.15 for p,q in zip(combined[45:],real)), 'Anchoring exceeded source shape budget'
    save('data/anhui-anchored.json',{'paths':[svg_path(p) for p in combined],'seams':seams})
    print('Cross pairs',[(data['regions'][a]['name'],regions[b-45]['name']) for a,b in sorted(adjacent(combined)) if a<45<=b],flush=True)
def finish():
    d,refs,fixed,_=inputs();r,raw,_=ah_source();a=load('data/anhui-anchored.json');initial=[geometry_from_path(p) for p in a['paths']]
    result,report=soften(initial,fixed+bodies(raw),locked_count=45,expected_topology=topology_signature(initial),split_rings=True,first_improvement=True)
    if any(acute_vertices(p) for p in result[45:]):
        result,fine=soften(result,fixed+bodies(raw),locked_count=45,expected_topology=topology_signature(initial),split_rings=True,first_improvement=True,detail_steps=(.5,1,2,4,6,8));report['fine']=fine
    save('data/anhui-finished.json',{'paths':[svg_path(p) for p in result],'report':report,'seams':a['seams']})
    print('Finished',[(rr['name'],turns(p),len(acute_vertices(p))) for rr,p in zip(r,result[45:])],flush=True)
def compact():
    from compact_lobes import optimize
    from simplify_shared import objective,boundary,shared_routes
    d,refs,fixed,_=inputs();r,raw,_=ah_source();a=load('data/anhui-finished.json');initial=[geometry_from_path(p) for p in a['paths']]
    allowed=[v["point"] for p in initial for v in acute_vertices(p)]
    assert all(unary_union(fixed).boundary.distance(Point(v))<1e-7 for p in initial[45:] for v in [x["point"] for x in acute_vertices(p)])
    names=[rr['name'] for rr in refs+r]
    result,report=optimize(initial,fixed+bodies(raw),names,target_names=names[45:],objective_fn=objective,boundary_fn=boundary,limit=18,chain_routes=shared_routes,locked_count=45,expected_topology=topology_signature(initial),allowed_acute_points=allowed,checkpoint_path=ROOT/"data/anhui-compact-checkpoint.json",first_improvement=True)
    report["inheritedAcutePoints"]=allowed
    save('data/anhui-final.json',{'paths':[svg_path(p) for p in result],'report':report,'seams':a['seams']})
def emit():
    from island_groups import fit_detached_island
    from simplify_shared import pair_stats
    from xml.sax.saxutils import escape
    data,old_refs,fixed,fixed_sha=inputs();regions,raw,source_sha=ah_source();refs=old_refs+regions
    a=load('data/anhui-final.json');polys=[geometry_from_path(p) for p in a['paths']];mainland=list(polys[45:]);islands=[]
    for i,p in enumerate(raw):
        parts=sorted(polygon_parts(p),key=lambda g:-g.area)
        for part in parts[1:]:
            if part.area<100 or part.area/p.area<.05:continue
            island,meta=fit_detached_island(part,unary_union(polys));polys[45+i]=unary_union([polys[45+i],island]);islands.append({'city':regions[i]['name'],**meta,'path':svg_path(island)})
    out=[]
    for i,(r,p) in enumerate(zip(refs,polys)):out.append({'id':r['id'],'name':r['name'],'province':r['province'],**({'fullName':r['fullName'],'administrativeType':r['administrativeType']} if 'fullName' in r else {}),'variants':{'east':{'path':svg_path(p),'label':label_for(max(polygon_parts(p),key=lambda g:g.area),data['regions'][i]['variants']['east']['label'] if i<45 else r['label'])}}})
    for r in refs:r['label']=label_for(geometry_from_path(r['path']),r['label'])
    metrics=[{'name':r['name'],'turns':turns(p),'parts':len(polygon_parts(p)),'acuteAngles':acute_vertices(p),'mainlandIou':q.intersection(real).area/q.union(real).area,'mainlandAreaError':abs(q.area-real.area)/real.area,'fullSourceIou':p.intersection(original).area/p.union(original).area} for r,p,q,real,original in zip(regions,polys[45:],mainland,bodies(raw),raw)]
    final_seams=[]
    for item in a['seams']:
    ...
    preparation={'omittedParts' :sum(len(polygon_parts(p))-1 for p in raw),'filledHoles':[{'name':r['name'],'area':Polygon(h).area} for r,holes in zip(regions,filled_holes(raw)) for h in holes],'omittedEnclaveHoleCoverage':.999,'preAnchorMinimumIou':.82,'sourceArcGapBridge':{'maximumLength':6,'maximumDistance':2}}
    report={'version':'0.13.0','frozenEastSha256':fixed_sha,'anhuiSourceSha256':source_sha,'retrieved':'2026-09-09','preparation':preparation,'cities':metrics,'seams':final_seams,'initialSeams':a['seams'],'islands':islands,'sharedPairs':pair_stats(polys,[r['name'] for r in refs]),'optimization':a['report'],'inheritedAcutePoints':a['report']['inheritedAcutePoints']}
    result={'version':'0.13.0','defaultVariant':'east','regions':out,'variants':{'east':{'outline':svg_path(unary_union(polys))}}}
    (ROOT/'dist/expanded-data.js').write_text('const MAP_DATA = '+json.dumps(result,ensure_ascii=False)+';\nconst REFERENCE_MAP = '+json.dumps({'regions':refs},ensure_ascii=False)+';\n')
    save('dist/anhui-report.json',report)
    svg=['<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1660 1520"><title>江西、浙江、福建、湖南与安徽真实区划参考</title><rect width="1660" height="1520" fill="white"/><g transform="translate(500 530)">']
    ...
    svg.append('</g></svg>');(ROOT/'dist/expanded-reference.svg').write_text(''.join(svg))
