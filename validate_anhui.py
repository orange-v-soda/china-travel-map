"""Independent checks on delivered five-province geometry."""
import json
import shapely
...
from build_anhui import inputs,ah_source,bodies
from simplify_boundaries import turns
from simplify_shared import pair_stats
from soften_angles import acute_vertices

def validate():
    s=(ROOT/'dist/expanded-data.js').read_text();d=json.loads(s.split(' = ',1)[1].split(';\n')[0]);ref=json.loads(s.split('const REFERENCE_MAP = ',1)[1].rstrip(';\n'))['regions'];report=json.loads((ROOT/'dist/anhui-report.json').read_text())
    old,oldref,fixed,sha=inputs();regions,raw,source_sha=ah_source();real=bodies(raw)
    assert sha==report['frozenEastSha256'] and source_sha==report['anhuiSourceSha256']
    polys=[parse(r['variants']['east']['path']) for r in d['regions']];assert len(polys)==61 and len({r['id'] for r in d['regions']})==61
    ...
    for i,r in enumerate(oldref):
        ...
        for j,q in enumerate(real):
            if q.boundary.intersection(p.boundary.buffer(.05)).length>3:cross.add((i,45+j))
    assert adjacent(polys)==adjacent(fixed)|{(i+45,j+45) for i,j in adjacent(real)}|cross
    assert topology_signature(polys[45:])==topology_signature(real)
    ...
    for i,(r,p) in enumerate(zip(d['regions'],polys)):
        ...
        if i<45:assert r['variants']['east']['label']==old['regions'][i]['variants']['east']['label']
        ...
        for rr in rings(p):
            ...
            assert all(0<=x+500<=1660 and 140<=y+530<=1440 for x,y in rr)
    assert set(actual)==allowed and len(actual)==len(allowed)
    for i,(p,r) in enumerate(zip(polys[45:],real)):
    ...
    assert report['preparation']['omittedParts']==3
    ...
    assert len(ref)==61
    assert all(parse(r['path']).equals(q) for r,q in zip(ref[45:],raw))
    ...
    print(json.dumps({'cities':61,'frozenCities':45,'crossProvincePairsAdded':len(cross),'adjacencies':len(adjacent(polys)),'overlaps':0,'inheritedAcuteAngles':len(actual),'anhuiInheritedAcuteAngles':sum(len(acute_vertices(p)) for p in polys[45:]),'unexpectedAcuteAngles':0,'anhuiTurns':[turns(p) for p in polys[45:]]},ensure_ascii=False))
