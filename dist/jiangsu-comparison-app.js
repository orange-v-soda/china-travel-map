'use strict';
const REFERENCE_MAP={regions:MAP_DATA.regions.map(r=>({...r,path:r.variants.east.path,label:r.variants.east.label}))};
...
referenceSvg.querySelector('#reference-map-title').textContent='新增江苏后的基础抽象图';
referenceSvg.querySelector('#reference-map-desc').textContent='均衡前的抽象地图，与右侧同步高亮和填色。';
referenceSvg.querySelector('#reference-map-subtitle').textContent='均衡前 / BASELINE';
referenceSvg.querySelector('#reference-map-note').textContent='基础抽象图 · 几何保持原样';
referenceSvg.querySelector('#reference-outline').setAttribute('d',MAP_DATA.variants.east.outline);
...
document.querySelector('#export').addEventListener('click',async()=>{
 ...
 try{
  ...
  const copy=svg.cloneNode(true);copy.setAttribute('viewBox','0 0 1660 2040');copy.setAttribute('width','3320');copy.setAttribute('height','4080');copy.querySelectorAll('[tabindex]').forEach(el=>{el.removeAttribute('tabindex');el.removeAttribute('role');});
  ...
  const canvas=document.createElement('canvas');canvas.width=3320;canvas.height=4080;const ctx=canvas.getContext('2d');if(!ctx)throw new Error('浏览器不支持图片导出');ctx.drawImage(img,0,0,3320,4080);
  const blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/png'));if(!blob)throw new Error('无法生成 PNG');
  const downloadUrl=URL.createObjectURL(blob);const a=document.createElement('a');a.href=downloadUrl;a.download='中国旅行地图-江苏.png';document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(downloadUrl),60000);status.textContent='已生成完整图片（3320 × 4080）';
...
const groups={jiangsu:['南京','无锡','徐州','常州','苏州','南通','连云港','淮安','盐城','扬州','镇江','泰州','宿迁'],shangrao:['上饶','景德镇','鹰潭','抚州','衢州','黄山','南平'],macau:['澳门','珠海','中山'],hongkong:['香港','深圳','东莞'],shanghai:['上海','苏州','南通','嘉兴','宁波','舟山'],delta:['香港','澳门','广州','佛山','中山','珠海','深圳','东莞','惠州','江门'],minnan:['厦门','漳州','泉州'],chaoshan:['汕头','潮州','揭阳'],islands:['舟山','宁波']};
function focusMap(){
 const key=document.querySelector('#focus').value;let box='0 0 1660 2040';
 if(key!=='all'){
  const pts=MAP_DATA.regions.filter(r=>groups[key].includes(r.name)).flatMap(r=>[...Object.values(r.variants),REFERENCE_MAP.regions.find(x=>x.id===r.id)].flatMap(v=>Array.from(v.path.matchAll(/(-?\d+(?:\.\d+)?),(-?\d+(?:\.\d+)?)/g),m=>[Number(m[1])+500,Number(m[2])+690])));
...
fetch('jiangsu-balanced-report.json').then(r=>{if(!r.ok)throw new Error('report unavailable');return r.json();}).then(report=>{
