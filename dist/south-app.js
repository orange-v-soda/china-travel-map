...
referenceSvg.querySelector('#reference-map-title').textContent='江西、浙江、福建、湖南、安徽与广东真实区划参考图';
...
document.querySelector('#export').addEventListener('click',async()=>{
 ...
 try{
  ...
  const copy=svg.cloneNode(true);copy.setAttribute('width','3320');copy.setAttribute('height','3760');copy.querySelectorAll('[tabindex]').forEach(el=>{el.removeAttribute('tabindex');el.removeAttribute('role');});
  ...
  const downloadUrl=URL.createObjectURL(blob);const a=document.createElement('a');a.href=downloadUrl;a.download='中国旅行地图-江西、浙江、福建、湖南、安徽与广东.png';document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(downloadUrl),60000);status.textContent='已生成完整图片（3320 × 3760）';
