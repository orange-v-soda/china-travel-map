...
referenceSvg.querySelector('#reference-map-title').textContent='当前抽象地图';
referenceSvg.querySelector('#reference-map-desc').textContent='均衡前的抽象地图，同步高亮并标记对应城市。';
referenceSvg.querySelector('#reference-map-subtitle').textContent='当前版本 / BASELINE';
referenceSvg.querySelector('#reference-map-note').textContent='当前抽象版 · 几何保持原样';
...
document.querySelector('#export').addEventListener('click',async()=>{
 ...
 try{
  ...
  const downloadUrl=URL.createObjectURL(blob);const a=document.createElement('a');a.href=downloadUrl;a.download='中国旅行地图-面积均衡对照.png';document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(downloadUrl),60000);status.textContent='已生成完整图片（3320 × 3760）';
...
const groups={shangrao:['上饶','景德镇','鹰潭','抚州','衢州','黄山','南平'],delta:['广州','佛山','中山','珠海','深圳','东莞','惠州','江门'],minnan:['厦门','漳州','泉州'],chaoshan:['汕头','潮州','揭阳'],islands:['舟山','宁波']};
function focusMap(){
...
document.querySelector('#variant').addEventListener('change',e=>{
 ...
 document.querySelector('#map-note').textContent=mode==='balanced'?'轮廓结构保持 · 区划面积温和调整':'当前抽象版 · 几何保持原样';
});
let balanceMetrics=new Map();
fetch('balanced-report.json').then(r=>{if(!r.ok)throw new Error('report unavailable');return r.json();}).then(report=>{
 balanceMetrics=new Map(report.cities.map(r=>[r.id,r]));
 document.querySelector('#balance-summary').textContent=`最大／最小图面面积：${report.beforeRatio.toFixed(2)} 倍 → ${report.afterRatio.toFixed(2)} 倍。保留全部区划连接关系与原有转角，目标均衡强度 α＝0.75。`;
}).catch(()=>{document.querySelector('#balance-summary').textContent='目标均衡强度 α＝0.75。选择局部范围，比较小区划与周围轮廓的变化。';});
