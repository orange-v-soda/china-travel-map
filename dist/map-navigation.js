...
(() => {
 ...
 let view={x:0,y:0,k:1},gesture=null,suppressUntil=0,labelsHidden=false;
 ...
 function apply(){
  ...
  // Hysteresis avoids flicker while hovering around the detail threshold.
  if(view.k>=5)labelsHidden=true;else if(view.k<=4.5)labelsHidden=false;
  const labels=map.querySelector('#labels');
  if(labels){labels.setAttribute('opacity',labelsHidden?'0':'1');labels.setAttribute('aria-hidden',String(labelsHidden));}
 ...
 map.addEventListener('map-focus',e=>{const b=e.detail;view={x:b.x,y:b.y,k:Math.min(W/b.width,H/b.height)};apply();});
 apply();
 document.querySelector('#zoom-jiangxi')?.click();
})();
