...
referenceSvg.querySelector('#reference-map-title').textContent='江西、浙江、福建、湖南与安徽真实区划参考图';
...
document.querySelector('#export').addEventListener('click',async()=>{
 ...
 try{
  ...
  const copy=svg.cloneNode(true);copy.setAttribute('width','3320');copy.setAttribute('height','3040');copy.querySelectorAll('[tabindex]').forEach(el=>{el.removeAttribute('tabindex');el.removeAttribute('role');});
  ...
  const canvas=document.createElement('canvas');canvas.width=3320;canvas.height=3040;const ctx=canvas.getContext('2d');if(!ctx)throw new Error('浏览器不支持图片导出');ctx.drawImage(img,0,0,3320,3040);
  const blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/png'));if(!blob)throw new Error('无法生成 PNG');
  const downloadUrl=URL.createObjectURL(blob);const a=document.createElement('a');a.href=downloadUrl;a.download='中国旅行地图-江西、浙江、福建、湖南与安徽.png';document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(downloadUrl),60000);status.textContent='已生成完整图片（3320 × 3040）';
