"""Independent checks on delivered three-province geometry."""
import json
import shapely
...
from build_fujian import inputs,fj_source,bodies
from simplify_boundaries import turns
from simplify_shared import pair_stats
from soften_angles import acute_vertices

def validate():
    s=(ROOT/'dist/southeast-data.js').read_text();d=json.loads(s.split(' = ',1)[1].split(';\n')[0]);ref=json.loads(s.split('const REFERENCE_MAP = ',1)[1].rstrip(';\n'))['regions'];report=json.loads((ROOT/'dist/fujian-report.json').read_text())
    old,oldref,fixed,sha=inputs();regions,raw,source_sha=fj_source();real=bodies(raw)
    assert sha==report['frozenEastSha256'] and source_sha==report['fujianSourceSha256']
    polys=[parse(r['variants']['east']['path']) for r in d['regions']];assert len(polys)==31 and len({r['id'] for r in d['regions']})==31
    ...
    for i,r in enumerate(oldref):
        ...
        for j,q in enumerate(real):
            if q.boundary.intersection(p.boundary.buffer(.05)).length>3:cross.add((i,22+j))
    assert len(cross)==10
    assert adjacent(polys)==adjacent(fixed)|{(i+22,j+22) for i,j in adjacent(real)}|cross
    kept=[]
    for p in raw:kept.append(unary_union([q for j,q in enumerate(sorted(polygon_parts(p),key=lambda g:-g.area)) if j==0 or (q.area>=100 and q.area/p.area>=.05)]))
    assert topology_signature(polys[22:])==topology_signature(kept)
    ...
    allowed=set(map(tuple,report['inheritedAcutePoints']));assert allowed=={(495.,398.),(728.5,417.5)}
    actual=[]
    for i,(r,p) in enumerate(zip(d['regions'],polys)):
        assert p.contains(Point(r['variants']['east']['label']))
        if i<22:assert r['variants']['east']['label']==old['regions'][i]['variants']['east']['label']
        ...
        for rr in rings(p):
            ...
            assert all(0<=x+40<=1160 and 140<=y+140<=1080 for x,y in rr)
    assert set(actual)==allowed and len(actual)==2
    for i,(p,r) in enumerate(zip(polys[22:],real)):
    ...
    print(json.dumps({'cities':31,'frozenCities':22,'crossProvincePairsAdded':len(cross),'adjacencies':len(adjacent(polys)),'overlaps':0,'inheritedAcuteAngles':len(actual),'newAcuteAngles':0,'fujianTurns':[turns(p) for p in polys[22:]]},ensure_ascii=False))
