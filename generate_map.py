...
def generate():
    ...
    for i,r in enumerate(reference['regions']):
        regions.append({'id':r['id'],'name':r['name'],'province':r.get('province'),'variants':{mode:{'path':svg_path(polys[i]),'label':label_for(polys[i],r['label'])} for mode,polys in variants.items()},'neighbors':sorted(reference['regions'][j if i==k else k]['id'] for k,j in expected if i in (k,j))})
    data={'version':'0.4.0','basis':'iterative-octilinear','sourceSha256':reference['sha256'],'title':reference.get('title',''),'shortName':reference.get('shortName',''),'provinces':reference.get('provinceNames',[]),'canvas':reference.get('canvas',{'width':760,'height':860}),'adjacencyCount':len(expected),'tolerance':TOLERANCE,'iterationTolerance':ITERATION_TOLERANCE,'grid':GRID,'defaultVariant':list(variants)[-1],'convergence':convergence,'iterations':logs,'regions':regions,'variants':{mode:{'outline':svg_path(unary_union(polys)),**stats(real,polys)} for mode,polys in variants.items()},'sourceVertices':int(shapely.get_num_coordinates(real).sum())}
