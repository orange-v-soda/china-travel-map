"""Independent checks on delivered four-province geometry."""
import json
import shapely
...
from build_hunan import inputs,hn_source,bodies
from simplify_boundaries import turns
from simplify_shared import pair_stats
from soften_angles import acute_vertices

def validate():
    s=(ROOT/'dist/atlas-data.js').read_text();d=json.loads(s.split(' = ',1)[1].split(';\n')[0]);ref=json.loads(s.split('const REFERENCE_MAP = ',1)[1].rstrip(';\n'))['regions'];report=json.loads((ROOT/'dist/hunan-report.json').read_text())
    old,oldref,fixed,sha=inputs();regions,raw,source_sha=hn_source();real=bodies(raw)
    assert sha==report['frozenEastSha256'] and source_sha==report['hunanSourceSha256']
    polys=[parse(r['variants']['east']['path']) for r in d['regions']];assert len(polys)==45 and len({r['id'] for r in d['regions']})==45
    ...
    for i,r in enumerate(oldref):
        ...
        for j,q in enumerate(real):
            if q.boundary.intersection(p.boundary.buffer(.05)).length>3:cross.add((i,31+j))
    assert len(cross)==8
    assert adjacent(polys)==adjacent(fixed)|{(i+31,j+31) for i,j in adjacent(real)}|cross
    assert topology_signature(polys[31:])==topology_signature(real)
    ...
    for i,(r,p) in enumerate(zip(d['regions'],polys)):
        ...
        if i<31:assert r['variants']['east']['label']==old['regions'][i]['variants']['east']['label']
        ...
        for rr in rings(p):
            ...
            assert all(0<=x+500<=1660 and 140<=y+140<=1080 for x,y in rr)
    assert set(actual)==allowed and len(actual)==len(allowed)
    for i,(p,r) in enumerate(zip(polys[31:],real)):
    ...
    assert report['preparation']['omittedParts']==2
    assert len(report['preparation']['filledHoles'])==1 and report['preparation']['filledHoles'][0]['name']=='怀化'
    ...
    print(json.dumps({'cities':45,'frozenCities':31,'crossProvincePairsAdded':len(cross),'adjacencies':len(adjacent(polys)),'overlaps':0,'inheritedAcuteAngles':len(actual),'newAcuteAngles':0,'hunanTurns':[turns(p) for p in polys[31:]]},ensure_ascii=False))
