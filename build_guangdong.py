"""Extend the accepted eastern map with Guangdong on the same shared network."""
...
def base():
    ...
    result,report=simplify(initial,real,18,minimum_iou=.82,first_improvement=True,checkpoint_path=ROOT/"data/guangdong-base-checkpoint.json")
    report['initialCoverageTolerance']=tolerance
    save('data/guangdong-base.json',{'paths':[svg_path(p) for p in result],'report':report})
...
def anchor(base_path="data/guangdong-base.json",output_path="data/guangdong-anchored.json"):
    from itertools import combinations
    data,refs,fixed,_=inputs();regions,raw,_=gd_source();real=bodies(raw)
    fixed_union=max(polygon_parts(unary_union(fixed)),key=lambda p:p.area)
    old_boundary=unary_union(bodies([geometry_from_path(r['path']) for r in refs])).boundary
    ...
    for i,r in enumerate(refs):
        boundary=max(polygon_parts(geometry_from_path(r['path'])),key=lambda p:p.area).boundary.buffer(.05)
...
def finish():
    d,refs,fixed,_=inputs();r,raw,_=gd_source();a=load('data/guangdong-anchored.json');initial=[geometry_from_path(p) for p in a['paths']]
    result,report=soften(initial,fixed+bodies(raw),locked_count=61,expected_topology=topology_signature(initial),split_rings=True,first_improvement=True,checkpoint_path=ROOT/"data/guangdong-angle-checkpoint.json")
    if any(acute_vertices(p) for p in result[61:]):
        result,fine=soften(result,fixed+bodies(raw),locked_count=61,expected_topology=topology_signature(initial),split_rings=True,first_improvement=True,detail_steps=(.5,1,2,4,6,8),checkpoint_path=ROOT/"data/guangdong-fine-checkpoint.json");report['fine']=fine
    save('data/guangdong-finished.json',{'paths':[svg_path(p) for p in result],'report':report,'seams':a['seams']})
    print('Finished',[(rr['name'],turns(p),len(acute_vertices(p))) for rr,p in zip(r,result[61:])],flush=True)
def compact():
    from compact_lobes import optimize
    from simplify_shared import objective,boundary,shared_routes
    d,refs,fixed,_=inputs();r,raw,_=gd_source();a=load('data/guangdong-finished.json');initial=[geometry_from_path(p) for p in a['paths']]
    allowed=[v["point"] for p in initial for v in acute_vertices(p)]
    assert all(unary_union(fixed).boundary.distance(Point(v))<1e-7 for p in initial[61:] for v in [x["point"] for x in acute_vertices(p)])
    names=[rr['name'] for rr in refs+r]
    result,report=optimize(initial,fixed+bodies(raw),names,target_names=names[61:],objective_fn=objective,boundary_fn=boundary,limit=18,chain_routes=shared_routes,locked_count=61,expected_topology=topology_signature(initial),allowed_acute_points=allowed,checkpoint_path=ROOT/"data/guangdong-compact-checkpoint.json",first_improvement=True)
    report["inheritedAcutePoints"]=allowed
    save('data/guangdong-final.json',{'paths':[svg_path(p) for p in result],'report':report,'seams':a['seams']})
def emit():
    from island_groups import fit_detached_island
    from simplify_shared import pair_stats
    from xml.sax.saxutils import escape
    data,old_refs,fixed,fixed_sha=inputs();regions,raw,source_sha=gd_source();refs=old_refs+regions
    a=load('data/guangdong-final.json');polys=[geometry_from_path(p) for p in a['paths']];mainland=list(polys[61:]);islands=[]
    for i,p in enumerate(raw):
        parts=sorted(polygon_parts(p),key=lambda g:-g.area)
        for part in parts[1:2]:
            if part.area<100:continue
            island,meta=fit_detached_island(part,unary_union(polys));polys[61+i]=unary_union([polys[61+i],island]);islands.append({'city':regions[i]['name'],**meta,'path':svg_path(island)})
    out=[]
    for i,(r,p) in enumerate(zip(refs,polys)):out.append({'id':r['id'],'name':r['name'],'province':r['province'],**({'fullName':r['fullName'],'administrativeType':r['administrativeType']} if 'fullName' in r else {}),'variants':{'east':{'path':svg_path(p),'label':label_for(max(polygon_parts(p),key=lambda g:g.area),data['regions'][i]['variants']['east']['label'] if i<61 else r['label'])}}})
    for r in refs:r['label']=label_for(geometry_from_path(r['path']),r['label'])
    metrics=[{'name':r['name'],'turns':turns(p),'mainlandTurns':turns(q),'parts':len(polygon_parts(p)),'acuteAngles':acute_vertices(p),'mainlandIou':q.intersection(real).area/q.union(real).area,'mainlandAreaError':abs(q.area-real.area)/real.area,'fullSourceIou':p.intersection(original).area/p.union(original).area} for r,p,q,real,original in zip(regions,polys[61:],mainland,bodies(raw),raw)]
    final_seams=[]
    for item in a['seams']:
    ...
    preparation={'sourceMinorParts':sum(len(polygon_parts(p))-1 for p in raw),'omittedParts':sum(len(polygon_parts(p))-1 for p in raw)-len(islands),'retainedIslandRule':{'maximumPerCity':1,'minimumArea':100},'filledHoles':[{'name':r['name'],'area':Polygon(h).area} for r,holes in zip(regions,filled_holes(raw)) for h in holes],'omittedEnclaveHoleCoverage':.999,'initialCoverageTolerance':load('data/guangdong-base.json')['report']['initialCoverageTolerance'],'sourceDetailRestoration':load('data/guangdong-anchored.json')['sourceDetailRestoration'],'preAnchorMinimumIou':.82,'sourceArcGapBridge':{'maximumLength':6,'maximumDistance':2}}
    report={'version':'0.14.0','frozenEastSha256':fixed_sha,'guangdongSourceSha256':source_sha,'retrieved':'2026-09-10','preparation':preparation,'cities':metrics,'seams':final_seams,'initialSeams':a['seams'],'islands':islands,'sharedPairs':pair_stats(polys,[r['name'] for r in refs]),'optimization':a['report'],'inheritedAcutePoints':a['report']['inheritedAcutePoints']}
    result={'version':'0.14.0','defaultVariant':'east','regions':out,'variants':{'east':{'outline':svg_path(unary_union(polys))}}}
    (ROOT/'dist/south-data.js').write_text('const MAP_DATA = '+json.dumps(result,ensure_ascii=False)+';\nconst REFERENCE_MAP = '+json.dumps({'regions':refs},ensure_ascii=False)+';\n')
    save('dist/guangdong-report.json',report)
    svg=['<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1660 1880"><title>江西、浙江、福建、湖南、安徽与广东真实区划参考</title><rect width="1660" height="1880" fill="white"/><g transform="translate(500 530)">']
    ...
    svg.append('</g></svg>');(ROOT/'dist/south-reference.svg').write_text(''.join(svg))
    print('Delivery',metrics,'islands',islands,flush=True)
if __name__=='__main__':
    import fcntl,os
    generation_lock=(ROOT/'data/guangdong-generation.lock').open('a+')
    try:fcntl.flock(generation_lock,fcntl.LOCK_EX|fcntl.LOCK_NB)
    except BlockingIOError:raise SystemExit('Guangdong generation is already running; wait for its saved stage output')
