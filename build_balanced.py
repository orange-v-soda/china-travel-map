...
def run(input_path="dist/south-data.js",output_prefix="balanced",minimum_median_fraction=0.,alphas=None,max_iterations=400):
    ...
    oi=np.array(owners);ra=np.concatenate(rings);rb=np.concatenate([np.roll(rr,-1) for rr in rings]);owneredge=np.concatenate([np.full(len(rr),i) for rr,i in zip(rings,owners)]);partedge=np.concatenate([np.full(len(rr),i) for i,rr in enumerate(rings)]);counts=np.bincount(owneredge,minlength=count)
    ...
    def geometries(x):
        ...
        pp=[MultiPolygon([p for p,j in zip(ff,owners) if i==j]) if owners.count(i)>1 else ff[owners.index(i)] for i in range(count)]
        return pp
    ...
    for alpha in (alphas or [.95,.90,.85,.80,.75]):
        target=areas**alpha
        if minimum_median_fraction:
            # Introduce the readability floor gradually alongside compression.
            progress=(1-alpha)/(1-ALPHA)
            normalized=target*areas.sum()/target.sum()
            floor=np.median(normalized)*minimum_median_fraction
            final_floor=np.maximum(normalized,floor)
            target=normalized*(final_floor/normalized)**progress
        target*=areas.sum()/target.sum();scale=np.sqrt(target/areas);ptarget=partareas*target[oi]/areas[oi]
        ...
        def loss(z):
            ...
            err=(pa-ptarget)/ptarget;weights=1/np.bincount(oi,minlength=count)[oi];f=np.sum(weights*err**2);g=np.zeros_like(x)
        ...
        result=minimize(loss,z,jac=True,method='SLSQP',constraints=[constraint],options={'maxiter':max_iterations,'ftol':1e-9})
        step=1.
        while step>1/1024:
    ...
    if output_prefix!='balanced' and 'const REFERENCE_MAP = ' in source:
        payload+='const SOURCE_REFERENCE = '+source.split('const REFERENCE_MAP = ',1)[1]
    (ROOT/f'dist/{output_prefix}-data.js').write_text(payload)
    (ROOT/f'dist/{output_prefix}-report.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n')
